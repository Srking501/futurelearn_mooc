...
* Give insights on how "successful", is the overall 
  percentage of participants have passed or not, such that
  the University will be able to view the effectiveness of the course and 
  derive the best run in the online course.
...
stuff who keeps track of the online course and perhaps questions or feedback
...
[Newcastle University's success criteria](#determining-business-objectives-business-success-criteria) 
is `cyber.security.1_question.response`. Thus, for now, the focus will be on it.
...
```{r echo=FALSE}
sapply(cyber.security.1_question.response, class)
```
...
The column `submitted_at` is the representing the time the answer was submitted,
therefore,
...
it helps to capture the time series of the data for data modelling and 
exploration.
...
```{r}
# The Different Data in the Column `correct`
cyber.security.1_question.response %>% count(correct)
```
...
```{r}
# The Lack of Data in the Column `cloze_response`
cyber.security.1_question.response %>% count(cloze_response)
```

### II Data Quality Report - Missing or Inconsistent Data

```{r}
sprintf("Total of Empty `learner_id`: %s", nrow(cyber.security.1_question.response %>% 
  filter(learner_id == "") %>%
  select(learner_id)))
```
...
### Constructing Data: Derived Attributes

...

### Constructing Data: Generated Records
