...
* `week_number`, `step_number` and `question_number` are basically the 
    individual values that are being represented in `quiz_question`.
...
The changes are made under the file `munge/01-B.R`.
...
data types are character types. `learner_id` and `quiz_question` data types will
...
before_data_reformatting = cyber.security.1_question.response %>% 
    select(learner_id, quiz_question, correct)
...
The changes are made under the file `munge/01-C.R`.
