...
`r run6_percent`. However, as seen in the [exploring data](#exploring-data), 
after [data preparation](#data-preparation) where only the participants that
...
mentioned in [determining next steps](#determing-next-steps) section, is to 
...
purchased_cert = data.frame(purchased_statement = combined_runs_enrolments %>% filter(purchased_statement_at != "") %>% nrow(),
    ...
    rename("Bought Certificate" = purchased_statement,
           "Not Bought Certificate" = not_purchased_statement)
knitr::kable(purchased_cert, caption = "Total participants who bought or not bought certificates")
```
...
related to the 
[business success](#determining-business-objectives-business-success-criteria). 
